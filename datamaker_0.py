...
data_save_dir = 'datasets/multilabel_0/'
transformed_data_save_dir = 'datasets/transformed_multilabel/'
limiter = 0  # Minimum number of images per class
...
def visualize_data_distribution(dataset_dict, output_dir):
    ...
    #print(len(labels_per_image))
    #if len(labels_per_image) == 54489:
        #print(labels_per_image)
    plt.figure(figsize=(8, 6))
    plt.hist(labels_per_image, bins=range(1, max(labels_per_image) + 2))
    plt.xlabel('Number of Labels')
    plt.xticks(np.arange(1, max(labels_per_image) + 2))
    plt.ylabel('Number of Images')
