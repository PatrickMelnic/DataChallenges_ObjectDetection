...
data_save_dir = 'datasets/multilabel_1000/'
transformed_data_save_dir = 'datasets/transformed_multilabel/'
limiter = 1000  # Minimum number of images per class
...
def create_dataset_dict(temp_dict):
    ...
    for split, split_data in splits.items():
        dataset = Dataset.from_dict({
            'image_path': [item['image_path'] for item in split_data],
...
def transform_dataset_to_hf_format(dataset_dict):
    ...
    print_with_time("Creating new dataset")
...
def visualize_data_distribution(dataset_dict, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, '1000_images_per_class.png'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, '1000_labels_per_image.png'))
    plt.close()
...
def main():
    ...
    temp_dict = process_images(filtered_label_paths, data_dir)

    # Step 3: Load or create class index mapping
    print_with_time("Loading or creating class index mapping.")
    class_mapping_file = 'class_index_mapping_1000.json'
