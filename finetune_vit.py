...
def compute_metrics(eval_pred):
    ...
    # Convert predictions and labels to the correct format for evaluate module
    ...
    debug_info = {
        ...
        "predictions_example": preds[:10].tolist(),
        "references_shape": labels.shape,
        "references_dtype": str(labels.dtype),
        "references_example": labels[:10].tolist()
    }
    ...
    # Evaluate metrics
    accuracy_score = accuracy_metric.compute(predictions=preds, references=labels)
    precision_score = precision_metric.compute(predictions=preds, references=labels, average='micro')
    recall_score = recall_metric.compute(predictions=preds, references=labels, average='micro')
    f1_score = f1_metric.compute(predictions=preds, references=labels, average='micro')
