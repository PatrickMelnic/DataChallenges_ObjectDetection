...
for example in dataset['eval']:  # Ensure we are iterating over the eval split
    ...
    text_inputs = processor(text=[true_caption], return_tensors="pt", padding=True, truncation=True)
    with torch.no_grad():
...
for i, (image_features, true_caption, image_path) in enumerate(zip(image_features_list, true_captions, image_paths)):
    """
    Cosine Similarity:
...
for example in dataset['eval']:  # Ensure we are iterating over the eval split
    ...
    caption = example['label']  # Assuming 'label' contains the caption
    file_name = example['filename']
...
for i in range(len(tsne_text)):
    ...
    plt.text(tsne_text[i, 0], tsne_text[i, 1], captions[i], fontsize=9)
